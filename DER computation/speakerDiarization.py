...
def run_diarization(params, args, SAVED_MODEL_NAME, input_dir, wav_name, out_dir, observation_dim, embedding_per_second, overlap_rate):
    ...
    specs, intervals = load_data(os.path.join(input_dir, wav_name), embedding_per_second=embedding_per_second, overlap_rate=overlap_rate)
    ...
    for spk, timeDicts in speakerSlice.items():  # time map to origin wav(contains mute)
    ...
    reg_name = re.sub('/.*', '', wav_name[:-4])
    diarization = Annotation(...)
    with open(os.path.join(out_dir, wav_name[:-4] + '.rttm'), 'w') as f:
        for spk, timeDicts in speakerSlice.items():
            print('========= ' + str(spk) + ' =========')
            for timeDict in timeDicts:
                start = timeDict['start']  # time in miliseconds
                end = timeDict['stop']
                diarization[Segment(start / 1000, end / 1000)] = spk
                s = fmtTime(start)  # change point moves to the center of the slice
                e = fmtTime(end)
                print(s + ' ==> ' + e)
                f.write(f'SPEAKER {reg_name} 1 {float(start / 1000.0)} {float((end - start) / 1000.0)} '
                        f'<NA> <NA> spk{spk} <NA> <NA>\n')

    return diarization, speakerSlice

def groundtruth_annotation(groundtruth_dir, wav_name):
    with open(os.path.join(groundtruth_dir, wav_name[:-4] + '.rttm'), 'r') as f:
        rttm = f.read().splitlines()

    groundtruth = Annotation(...)
    for g in rttm:
        t = g.split()
        start = float(g.split()[3])
        offset = float(g.split()[4])
        label = g.split()[7]
        groundtruth[Segment(start, start + offset)] = label
